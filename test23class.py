# 클래스는 어떤 데이터와 그 데이터를 처리하는 기능이 서로 밀접하게 관련되어 있다면 하나로 묶어 처리할 수 있다.
# 이를 클래스로 구현 가능하다.
...
# 1) 거리는 나중에 KN, K-Means, Embedding 유사도 등과 연결되고,
# 2) 기울기는 선형회귀를 거쳐 미분, Gradient Desecent, 딥러니의 학습원리로 연결된다.
# 3) 첨도, 왜도가 큰 (편차가 큰 데이터...) 데이터를 로그변환하면 
#    분포 개선, 범위 차이 축소 등으로 인해 모델을 안정적으로 수행가능 
...
class CalcTest:
    ...
    def invers_trans(self, x_list:list[float]):
...
def main():
    ...
    reversed_data = ctest.invers_trans(data_log_scaled)
    print("역변환 자료 :", [round(val ,1) for val in reversed_data])
    print("역변환 자료:" , reversed.data_log_round)
